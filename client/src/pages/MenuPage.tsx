      ...
      <section className="py-20">
        <div className="content-canvas">
          ...
          {loading ? (
            ...
            <div className="py-16 text-center max-w-2xl mx-auto space-y-6 bg-[#1c1b1b] border border-[#2a2a2a] rounded-3xl p-8 sm:p-12 shadow-2xl">
              <div className="w-16 h-16 rounded-full bg-[#D32F2F]/20 border border-[#D32F2F] flex items-center justify-center mx-auto text-[#ffb955]">
                <Flame className="w-8 h-8 animate-pulse text-[#D32F2F]" />
              </div>
              <div className="space-y-2">
                <span className="font-sans text-xs uppercase font-bold tracking-[0.25em] text-[#ffb955]">
                  Secret Stone-Deck Calibration
                </span>
                <h3 className="font-headline text-3xl sm:text-4xl uppercase text-white">
                  {selectedHeatLevel !== null 
                    ? `New ${selectedHeatLevel === 1 ? "Warm" : selectedHeatLevel === 2 ? "Kick" : "Fiery"} Recipes Coming Soon!` 
                    : "New Season Drops Coming Soon!"}
                </h3>
                <p className="font-sans text-sm text-[#cdc0ad] leading-relaxed">
                  Our master pizzaiolos are currently crafting new small-batch recipes and deals for this spice tier. In the meantime, explore our signature favorites or reset your filters below.
                </p>
              </div>
              <div className="flex flex-wrap items-center justify-center gap-3 pt-2">
                <button 
                  onClick={() => { setSelectedCategory("all"); setSelectedHeatLevel(null); setSearchQuery(""); }}
                  className="bg-[#D32F2F] hover:bg-[#be123c] text-white font-headline text-lg uppercase px-8 py-3 rounded-full transition-all btn-press shadow-lg"
                >
                  View All Available Creations
                </button>
                {selectedHeatLevel !== null && (
                  <button 
                    onClick={() => setSelectedHeatLevel(null)}
                    className="bg-white/10 hover:bg-white/15 text-white font-headline text-lg uppercase px-6 py-3 rounded-full transition-all btn-press"
                  >
                    Clear Heat Dial Only
                  </button>
                )}
              </div>
            </div>
                ...
                return (
                  ...
                  >
                    ...
                    <div className="w-full lg:w-1/2 space-y-6">
                      ...
                      {/* What's Inside & Ingredients Detail Section */}
                      {(item.includesList || item.ingredients) && (
                        <div className="p-4 rounded-2xl bg-[#171717] border border-white/10 space-y-2.5 max-w-md">
                          <div className="flex items-center gap-1.5 text-xs font-mono font-bold text-[#ffb955] uppercase tracking-wider">
                            <Sparkles className="w-3.5 h-3.5 text-[#ffb955]" />
                            <span>
                              {item.category === "deals" 
                                ? "What's Included in Feast" 
                                : item.category === "drinks" 
                                ? "Beverage Details" 
                                : item.category === "desserts" 
                                ? "Dessert Details" 
                          ...
                          {/* Includes List */}
                          {item.includesList && item.includesList.length > 0 && (
                            <ul className="space-y-1.5 text-xs text-[#cdc0ad]">
                              {item.includesList.map((inc, i) => (
                          ...
                          {item.ingredients && item.ingredients.length > 0 && (
                            <div className="flex flex-wrap gap-1.5 pt-1.5 border-t border-white/5">
                              {item.ingredients.map((ing, i) => (
                                <span key={i} className="px-2.5 py-0.5 rounded-md bg-[#242424] text-[11px] font-sans text-[#e5e2e1] border border-white/5">
                                  {ing}
                                </span>
                              ))}
                            </div>
